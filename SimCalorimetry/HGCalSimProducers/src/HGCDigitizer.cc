...
namespace {
  ...
  void saveSimHitAccumulator_forPreMix(PreMixSimAccumulator& simResult,
    ...
    static_assert(nEnergies <= PHGCSimAccumulator::Data::energyMask + 1,
		  "PreMixSimAccumulator bit pattern needs to updated");
    ...
    //std::cout<<digiCollection<<"\t"<<simData.size()<<std::endl;
	...
	//if(nhits > 0)
	//  std::cout<<" ** aha "<<nhits<<"\t"<<vc.size()<<"\t"<<vt.size()<<std::endl;
	simResult.emplace_back_timing(id.rawId(), iSample, vc, vt);
      }
    }
    //std::cout<<"------ used savesimAccumulator_forPremix "<<std::endl;
  ...
  typedef std::vector<std::pair<float, float> >::iterator itr;
  typedef std::map<uint32_t, std::vector<std::pair<float, float> > > IdHit_Map;
  template <typename GEOM> void loadSimHitAccumulator_forPreMix(hgc::HGCSimHitDataAccumulator& simData,
							      hgc::HGCPUSimHitDataAccumulator& PUSimData,
							      const GEOM* geom, IdHit_Map& hitRefs_bx0,
							      const PreMixSimAccumulator& simAccumulator,
    ...
    //std::cout<<"---------------  loadSimHitAccumulator_forPreMix being called "<<minbiasFlag<<std::endl;
    const float minPackChargeLog = minCharge > 0.f ? std::log(minCharge) : -2;
    const float maxPackChargeLog = std::log(maxCharge);
    constexpr uint16_t base = 1 << PreMixSimAccumulator::SimHitCollection::sampleOffset;
	...
	//if(!minbiasFlag)
	  //myfile6.close();
	//std::cout<<"Before Calculating fireTDC "<<std::endl;
	if(minbiasFlag){
	  //std::cout<<"size of hitRefs_bx0["<<detId<<"] is "<<hitRefs_bx0[detId].size()<<std::endl;
...
void HGCDigitizer::finalizeEvent(edm::Event& e, edm::EventSetup const& es, CLHEP::HepRandomEngine* hre) {
  ...
  if (premixStage1_) {
    ...
    std::unique_ptr<PreMixSimAccumulator> simRecord;
    ...
    if(!pusimHitAccumulator_->empty()) {
      ...
      simRecord = std::make_unique<PreMixSimAccumulator>();
      saveSimHitAccumulator_forPreMix(*simRecord, *pusimHitAccumulator_, validIds_, premixStage1MinCharge_, premixStage1MaxCharge_, digiCollection_, nEvents_, premixStage1_);
    }
...
void HGCDigitizer::accumulate_forPreMix(edm::Event const& e, edm::EventSetup const& eventSetup, CLHEP::HepRandomEngine* hre) {
  //get inputs
...
void HGCDigitizer::accumulate_forPreMix(edm::Handle<edm::PCaloHitContainer> const& hits,
  ...
  for(int i=0; i<nchits; ++i) {
    ...
    //const float old_charge_term = hit.energy() * 1e6 * keV2fC * getCCE(geom, id, cce_);
...
void HGCDigitizer::accumulate_forPreMix(const PreMixSimAccumulator& simAccumulator, const bool& minbiasFlag) {
  ...
  if (nullptr != gHGCal_){
    ...
    loadSimHitAccumulator_forPreMix(*simHitAccumulator_, *pusimHitAccumulator_, gHGCal_, hitRefs_bx0, 
    ...
    loadSimHitAccumulator_forPreMix(*simHitAccumulator_, *pusimHitAccumulator_, gHcal_, hitRefs_bx0,
				  simAccumulator, premixStage1MinCharge_, premixStage1MaxCharge_, !weightToAbyEnergy, 
				  tdcForToAOnset, nEvents_, minbiasFlag, digiCollection_);}
