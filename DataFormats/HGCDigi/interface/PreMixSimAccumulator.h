#ifndef DataFormats_HGCDigi_PreMixSimAccumulator_h
#define DataFormats_HGCDigi_PreMixSimAccumulator_h
...
class PreMixSimAccumulator{
 public:
  ...
  public:
    // begin
    const_iterator(const PreMixSimAccumulator* ncc)
      : ncc_(ncc), iDet_(0), iData_(0), endData_(ncc->detIdSize_.empty() ? 0 : ncc->detIdSize_.front().size()) {}
    // end
    const_iterator(const PreMixSimAccumulator* ncc, unsigned int detSize, unsigned int dataSize)
  ...
  private:
    const PreMixSimAccumulator* ncc_;
    unsigned int iDet_;
    unsigned int iData_;
