#ifndef DataFormats_HGCDigi_CnTSimAccumulator_h
#define DataFormats_HGCDigi_CnTSimAccumulator_h
 ...
 public:
  ...
  public:
    ...
    : nhits_(accCharge.size()){
      //std::cout<<"### printing from CnT "<<nhits_<<std::endl;
  ...
  SimHitCollection(const SimHitCollection& simhitcollection)
    : nhits_(simhitcollection.nhits_), chargeArray_(simhitcollection.chargeArray_), timeArray_(simhitcollection.timeArray_){}
  ...
  void emplace_back_timing(unsigned int detId, unsigned short sampleIndex, const std::vector<unsigned short>& accCharge ,
  ...
  public:
    // begin
    const_iterator(const CnTSimAccumulator* ncc)
      : ncc_(ncc), iDet_(0), iData_(0), endData_(ncc->detIdSize_.empty() ? 0 : ncc->detIdSize_.front().size()) {}
    // end
    const_iterator(const CnTSimAccumulator* ncc, unsigned int detSize, unsigned int dataSize)
